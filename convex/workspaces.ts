...
export const remove = mutation({
    ...
    handler: async (ctx, { workspaceId }) => {
        ...
        const [ members, channels, conversations, messages, reactions ] = await Promise.all([
            ...
            ctx.db.query("messages").withIndex("by_workspace_id", q => q.eq("workspaceId", workspaceId)).collect(),
            ctx.db.query("reactions").withIndex("by_workspace_id", q => q.eq("workspaceId", workspaceId)).collect(),
        ]);
